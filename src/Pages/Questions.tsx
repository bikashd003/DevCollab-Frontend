    ...
    <>
      {searchResults.length === 0 ? (
        <div className="w-[35vw] grid grid-cols-1 md:grid-cols-2 gap-2">
          <div className="">
            <p>
              <strong>[tag]</strong> :search by tag
            </p>
            <p>
              <strong>title</strong> :search by title
            </p>
          </div>
          <div className="whitespace-nowrap">
            <p>
              <strong>user:1234</strong> :search by user
            </p>
          </div>
        </div>
      ) : (
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground dark:bg-dark-background dark:text-dark-foreground">
      <main className="md:px-8 p-8">
        <div className="grid grid-cols-12 gap-4">
          ...
          <section className="custom-scrollbar col-span-12 md:col-span-6  rounded-lg shadow-md p-4 overflow-y-auto max-h-[85vh]">
            <div className="flex flex-col gap-4 mb-4">
              <button
                className="w-fit self-end bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2 rounded-md"
                onClick={() => navigate('/questions/ask')}
              >
                Ask Question
              </button>
            ...
            <div className="grid gap-4">
              {questions?.map((question: Question) => (
                <QuestionCard
                  key={question.id}
                  question={question}
                  loading={searchTerm ? submitSearchLoading : loading}
                />
              ))}
            </div>
